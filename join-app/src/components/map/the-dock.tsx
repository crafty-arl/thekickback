          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between px-4 pt-4 pb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-sans text-[15px] font-semibold text-white/90">{selectedVenue.name}</span>
                <span className="rounded-full bg-white/[0.06] px-2 py-0.5 font-sans text-[9px] font-semibold tracking-wide text-white/25">UNCLAIMED</span>
              </div>
              <motion.button onClick={handleKBBack} whileTap={{ scale: 0.85 }} className="flex h-7 w-7 items-center justify-center rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" className="opacity-40"><path d="M18 6 6 18M6 6l12 12" /></svg>
              </motion.button>
            </div>
            <div className="px-4 pb-2">
              <div className="flex flex-wrap items-center gap-2">
                {selectedVenue.category && <span className="rounded-md border border-white/[0.06] bg-white/[0.04] px-2 py-1 font-sans text-[11px] capitalize text-white/30">{selectedVenue.category}</span>}
                {selectedVenue.neighborhood && <span className="font-sans text-[11px] text-white/20">{selectedVenue.neighborhood}</span>}
              </div>
              {selectedVenue.description && <p className="mt-2 font-sans text-[12px] leading-[1.5] text-white/30">{selectedVenue.description}</p>}
            </div>
            <div className="px-4 pb-4 pt-2">
              ...
              <p className="mt-2 text-center font-sans text-[10px] text-white/15">Own this spot? Set up your venue page, AI agent, and member system — free.</p>
            </div>
          </div>
        )}
