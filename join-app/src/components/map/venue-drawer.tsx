...
export function VenueDrawer({ venue, onClose }: VenueDrawerProps) {
  ...
  return (
    <>
      {/* Floating tab bar — always visible when venue selected */}
      <motion.div
        initial={{ y: 80, opacity: 0 }}
        animate={dismissed ? { y: 80, opacity: 0 } : { y: 0, opacity: 1 }}
        exit={{ y: 80, opacity: 0 }}
        transition={{ type: "spring", damping: 25, stiffness: 280 }}
        className="fixed inset-x-0 bottom-0 z-50 flex flex-col items-center"
        style={{ paddingBottom: "max(8px, env(safe-area-inset-bottom, 8px))" }}
      >
        {/* Venue name pill above tabs */}
        <motion.div
        ...
        >
          ...
          <span className="font-sans text-[10px] text-white/30">{pct}% full</span>
          <motion.button
            onClick={handleClose}
            whileTap={{ scale: 0.85 }}
          ...
          {TABS.map((tab) => {
            ...
            return (
              <motion.button
                ...
                style={{
                  color: isActive ? vibeColor : "rgba(255,255,255,0.4)",
                }}
              >
                {isActive && (
                  <motion.div
      ...
      {/* Chat drawer panel — slides up when a tab is active */}
      <AnimatePresence>
        {activeTab && (
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-40 flex flex-col overflow-hidden rounded-t-[24px] border-t border-white/[0.15]"
            style={{
              height: "50dvh",
              marginBottom: 90,
          ...
          >
            ...
            {/* Drag handle */}
            <div className="relative flex justify-center pb-1 pt-2.5" style={{ touchAction: "none" }}>
              <motion.div
            ...
            {/* Input */}
            <div
              className="relative flex items-center gap-2.5 px-4 pt-2 pb-3"
            >
              <input
                ref={inputRef}
                type="text"
